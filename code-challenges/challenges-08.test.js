...
const containsAnd = (arr) => {
  // Solution code here...
  let getAnd = arr.filter((n,i)=>{
    if(n.includes('and') ){return true}
  });
  return getAnd;
};
...
const filterStringsWithVowels = (arr) => {
  // Solution code here...
  let regPattern = /([aeiou])/g;
  
  let getWordsVowels = arr.filter((n,i)=>{
    // if(regPattern.test(n))
    if(/([aeiou])/g.test(n)){
      return true;
    }
  })
  return getWordsVowels;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  // Solution code here...
  let getArray = arr.filter((n,i)=>{
    if(!forbiddenValues.includes(n) ){return true}
  });
  return getArray;
};
