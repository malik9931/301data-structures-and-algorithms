...
const longestString = (arr) => {
  // Solution code here...
  let longist = arr[0];
  let longString = arr.map((num) => {
    longist = num.length > longist.length ? num : longist;
  });
  // console.log(arr.indexOf(longist));
  return arr.indexOf(longist);
};
...
const standardizePhoneNumbers = (arr) => {
  ...
  let newArr = arr.map((val) => {
    return val.substring(1, 4) + val.substring(6, 9) + val.substring(10);
  });
  return newArr;
};
...
const allHappy = (arr) => {
  ...
  console.log(newArr);
  let result = newArr.length == arr.length ? true : false;
  return result;
};
