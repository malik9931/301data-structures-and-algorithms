...
const count = (target, input) => {
  // Solution code here...
  let resultArr = [];
  let newArr = input.map((singleArr) => {
    let integerNum = singleArr.map((element) => {
      if (element == target) {
  ...
  let newArr = input.map((singleArr) => {
    ...
    let integerNum = singleArr.map((element) => {
      if ((element % 5 == 0) & (typeof element == "number")) {
        resultArr2.push(Math.pow(2, element));
      }
    });
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let sum = data.reduce((acc, val, idx) => {
...
let findShortest = (data) => {
  ...
  let arr = data.map((val) => {
    if (val.name.length < shortestName.length) {
